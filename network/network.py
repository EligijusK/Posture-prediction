...
class Network:
    def __init__(self, *, checkpoint: str = None, checkpoint_type: str = None, man_labels: LabelManager):
        ...
        _, _, label_cache, _ = man_labels.label_cache

        softmaxer = []
        def get_softmax(leaf):
            if "children" in leaf:
                start = leaf.offset
                end = leaf.offset + len(leaf.children)
                softmaxer.append([leaf.name, start, end])

                for ch in leaf.children:
                    get_softmax(ch)
        get_softmax(label_cache)
    ...
    def train(self, *, dataset: Dataset):
        ...
        def do_epoch(epoch):
            ...
            with summary_training as writer:
                ...
                for key in train_acc_total:
                    writer.add_scalar("accuracy/%s" % key, train_acc_total[key], epoch)
            ...
            if epoch >= args.testing_set:
                with summary_testing as writer:
                    ...
                    for key in test_acc_total:
                        writer.add_scalar("accuracy/%s" % key, test_acc_total[key], epoch)
