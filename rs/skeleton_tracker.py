...
class SkeletonTracker:
    def __init__(self, cloud_tracking_api_key=""):
        ...
        initialise_logging(sdk_path, CM_LogLevel.CM_LL_ERROR,
                           True, os.path.join(default_log_dir(), "logs"))
        ...
        if not cloud_tracking_api_key:
            ...
            print(
                "Initialising the Skeleton Tracking Pipeline with ReID based CLOUD tracking.")
            self.__tracker = TrackingContext(
    ...
    def track_skeletons(self, color_image):
        ...
        try:
            skeletons = self.__api.update_tracking(
                color_image, self.__tracker, skeletons, False)
